...
for router, configs_firewall in data.items():
	...
	router_firewall_config_file.write(
		...
		"ip6tables -A FORWARD -p udp -m multiport --dport 546,547 -j ACCEPT\n"
		"ip6tables -A OUTPUT -p udp -m multiport --dport 546,547 -j ACCEPT\n\n"

		"# Allow SNMP\n"
		"ip6tables -A INPUT -p udp -m udp --dport 161 -j ACCEPT\n"
		"ip6tables -A FORWARD -p udp -m udp --dport 161 -j ACCEPT\n"
		"ip6tables -A OUTPUT -p udp -m udp --dport 161 -j ACCEPT\n\n"
	...
	if "lans" in configs_firewall:
		...
		if "visitor" in configs_firewall:
			...
			"	do\n"
				...
				"	ip6tables -A FORWARD -p tcp -s fd00:$k:3:"+configs_firewall["visitor"]+"::/64 -d fd00:${a}:3:"+configs_firewall["suffixe_DNS"]+"/64 --destination-port 53 -j ACCEPT\n"
				"	ip6tables -A FORWARD -p tcp -s fd00:$k:3:"+configs_firewall["visitor"]+"::/64 -d fd00:${a}:3:"+configs_firewall["suffixe_DNS2"]+"/64 --destination-port 53 -j ACCEPT\n"
			"	done\n"
			)
