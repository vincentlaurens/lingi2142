...
for router, configs_firewall in data.items():
	...
	router_firewall_config_file.write(
		...
		"iptables -A INPUT -p icmpv6 --icmpv6-type 8 -s 0/0 -d fd00:200::b/48 -m state --state NEW,ESTABLISHED,RELATED -j ACCEPT"
		"iptables -A OUTPUT -p icmpv6 --icmpv6-type 0 -s fd00:200::b/48 -d 0/0 -m state --state ESTABLISHED,RELATED -j ACCEPT"
		...
		"#ip6tables -A INPUT -p icmpv6 --icmpv6-type 8 -m limit --limit 50/min -j ACCEPT\n"
		"#ip6tables -A OUTPUT -p icmpv6 --icmpv6-type 0 -m limit --limit 50/min -j ACCEPT\n"
	...
	router_firewall_config_file.write(
		"		ip6tables -A OUTPUT -p udp -d fd00:${a}:3:1000::1/64 --dport 53 -m state --state NEW,ESTABLISHED -j ACCEPT\n"
		"		ip6tables -A INPUT  -p udp -s fd00:${a}:3:1000::1/64 --sport 53 -m state --state ESTABLISHED     -j ACCEPT\n"
		"		ip6tables -A OUTPUT -p tcp -d fd00:${a}:3:1000::1/64 --dport 53 -m state --state NEW,ESTABLISHED -j ACCEPT\n"
		"		ip6tables -A INPUT -p tcp -s fd00:${a}:3:1000::1/64 --sport 53 -m state --state ESTABLISHED -j ACCEPT\n"
		"done\n"
