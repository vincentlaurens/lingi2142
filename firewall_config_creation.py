...
for router, configs_firewall in data.items():
	...
	router_firewall_config_file.write("#!/bin/bash\n"
						...
						"for a in 200 300"
						"do"
						"		ip6tables -A INPUT -i $H"+router+"-eth1 -p tcp --dport 22 -j ACCEPT\n"
						...
						if configs["bgp"] == "true":
							...
							"		done"
						"			ip6tables -A OUTPUT -p udp -d $ip --dport 53 -m state --state NEW,ESTABLISHED -j ACCEPT\n"
						"			ip6tables -A INPUT  -p udp -s $ip --sport 53 -m state --state ESTABLISHED     -j ACCEPT\n"
						"			ip6tables -A OUTPUT -p tcp -d $ip --dport 53 -m state --state NEW,ESTABLISHED -j ACCEPT\n"
						"			ip6tables -A INPUT -p tcp -s $ip --sport 53 -m state --state ESTABLISHED -j ACCEPT\n"
						"done"
