		...
		for lan, lan_configs in configs_firewall["lans"].items():
			...
			if "StudStaff" in configs_firewall:
				...
				router_firewall_config_file.write(
						...
						"ip6tables -A FORWARD -s fd00:${a}:3:"+lan_rules[0]+"::"+configs_firewall["router_id"]+"/64 -d fd00:${a}:3:"+lan_rules[1]+"::"+configs_firewall["router_id"]+"/64 -j DROP\n\n"
						"#Drop connexion from stud to stud\n"
						"ip6tables -A FORWARD -s fd00:${a}:3:"+lan_rules[0]+"::"+configs_firewall["router_id"]+"/64 -d fd00:${a}:3:"+lan_rules[0]+"::"+configs_firewall["router_id"]+"/64 -j DROP\n\n"
						...
						"ip6tables -A FORWARD -p tcp -s fd00:${a}:3:"+lan_rules[0]+"::"+configs_firewall["router_id"]+"/64  -d fd00:${a}:3:"+lan_rules[1]+"::"+configs_firewall["router_id"]+"/64 -m multiport --dports 25,110,143 -j ACCEPT\n\n"
						...
						"ip6tables -A INPUT -p tcp -s fd00:${a}:3:"+lan_rules[0]+"::"+configs_firewall["router_id"]+"/64  -d  --dport 443 -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT\n\n"
						"ip6tables -A INPUT -p tcp -s fd00:${a}:3:"+lan_rules[1]+"::"+configs_firewall["router_id"]+"/64  --dport 80 -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT\n"
						"ip6tables -A INPUT -p tcp -s fd00:${a}:3:"+lan_rules[1]+"::"+configs_firewall["router_id"]+"/64   --dport 8080 -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT\n"
						"ip6tables -A INPUT -p tcp -s fd00:${a}:3:"+lan_rules[1]+"::"+configs_firewall["router_id"]+"/64  -d  --dport 443 -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT\n\n"
						"ip6tables -A OUTPUT -p tcp -s fd00:${a}:3:"+lan_rules[0]+"::"+configs_firewall["router_id"]+"/64  --dport 80 -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT\n"
						"ip6tables -A OUTPUT -p tcp -s fd00:${a}:3:"+lan_rules[0]+"::"+configs_firewall["router_id"]+"/64   --dport 8080 -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT\n"
						"ip6tables -A OUTPUT -p tcp -s fd00:${a}:3:"+lan_rules[0]+"::"+configs_firewall["router_id"]+"/64  -d  --dport 443 -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT\n\n"
				...
				if "Monitoring" in configs_firewall:
					pprint(configs_firewall["router_id"])
					router_firewall_config_file.write(
					"#Allow SNMP for Monitoring LAN and mailbox protocols and SSH for check log for instance\n"
					"ip6tables -A INPUT -p tcp -d fd00:${a}:3:"+lan_rules[0]+"::"+configs_firewall["router_id"]+"/64 -m tcp --dport 161 -j ACCEPT\n"
					"ip6tables -A INPUT -p udp -d fd00:${a}:3:"+lan_rules[0]+"::"+configs_firewall["router_id"]+"/64 -m udp --dport 162 -j ACCEPT\n"
					"ip6tables -A INPUT -p tcp -d fd00:${a}:3:"+lan_rules[0]+"::"+configs_firewall["router_id"]+"/64 --dport 22 -j ACCEPT\n"	
