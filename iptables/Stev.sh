...
ip6tables -A INPUT -i Stev-eth1 -p tcp --destination-port 22 -j ACCEPT
ip6tables -A OUTPUT -o  Stev-eth1 -p udp --destination-port 33434:33524 -m state --state NEW -j ACCEPT
for a in 200 300
do
	#Restrict incoming SSH to the local network
	ip6tables -A INPUT -i Stev-eth1 -p tcp -s fd00:$a:3::3/48 --destination-port 22 -j ACCEPT
...
ip6tables -L > group3_cfg/Stev/log/iptables_log
echo [INFO SECURITY: Stev] Firewall set up!
