...
do
	...
	ip6tables -A INPUT -s fd00:$a:3:f044::/64 -p icmpv6 --icmpv6-type 134/0 -j REJECT
	ip6tables -A INPUT -s fd00:$a:3:f045::/64 -p icmpv6 --icmpv6-type 134/0 -j REJECT
	# Accept FTP between two Staff hosts or between two Studs hosts and between Staff and Stud and Stud and Staff
	ip6tables -A FORWARD -p tcp -s fd00:$a:3:f044::/64 -d fd00:$a:3:f044::/64 --destination-port 21  -j ACCEPT
	ip6tables -A FORWARD -p tcp -s fd00:$a:3:f045::/64 -d fd00:$a:3:f045::/64 --destination-port 21  -j ACCEPT
	ip6tables -A FORWARD -p tcp -s fd00:$a:3:f045::/64 -d fd00:$a:3:f044::/64 --destination-port 21  -j ACCEPT
	ip6tables -A FORWARD -p tcp -s fd00:$a:3:f044::/64 -d fd00:$a:3:f045::/64 --destination-port 21  -j ACCEPT
	# Allow SSH for students and staff members for $a
	ip6tables -A FORWARD -p tcp -s fd00:$a:3:f044::/64 --destination-port 22 -j ACCEPT
	ip6tables -A FORWARD -p tcp -s fd00:$a:3:f045::/64 --destination-port 22 -j ACCEPT
	ip6tables -A FORWARD -p tcp -s fd00:$a:3:f045::/64 -d fd00:$a:3:f044::/64 --destination-port 22 -j DROP

	ip6tables -A FORWARD -p tcp -s fd00:$a:3:f044::/64 -d fd00:$a:3:f045::/64 --destination-port 22 -j DROP

	# Allow SMTP for students and staff members for $a
	ip6tables -A FORWARD -p tcp -s fd00:$a:3:f044::/64 --destination-port 25 -j ACCEPT

	ip6tables -A FORWARD -p tcp -s fd00:$a:3:f045::/64 --destination-port 25 -j ACCEPT

	# Allow HTTP and HTTPS for students and staff members for $a
	ip6tables -A FORWARD -p tcp -s fd00:$a:3:f044::/64 --destination-port 80 -j ACCEPT

	ip6tables -A FORWARD -p tcp -s fd00:$a:3:f045::/64 --destination-port 80 -j ACCEPT

	ip6tables -A FORWARD -p tcp -s fd00:$a:3:f044::/64 --destination-port 443 -j ACCEPT

	ip6tables -A FORWARD -p tcp -s fd00:$a:3:f045::/64 --destination-port 443 -j ACCEPT

done

# Restrict incoming SSH to a specific network interface
ip6tables -A INPUT -i Carn-eth1 -p tcp --destination-port 22 -j ACCEPT
ip6tables -A OUTPUT -o  Carn-eth1 -p udp --destination-port 33434:33524 -m state --state NEW -j ACCEPT
for a in 200 300
do
	#Restrict incoming SSH to the local network
	ip6tables -A INPUT -i Carn-eth1 -p tcp -s fd00:$a:3::4/48 --destination-port 22 -j ACCEPT
...
ip6tables -L > group3_cfg/Carn/log/iptables_log
echo [INFO SECURITY: Carn] Firewall set up!
