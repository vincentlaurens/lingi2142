...
ip6tables -A FORWARD -s fd00:${a}:3:4::/64 -d fd00:${a}:3:1::/64 -j DROP
...
ip6tables -A FORWARD -s fd00:${a}:3:1::/64 -d fd00:${a}:3:1::/64 -j DROP

# Allow\Drop SSH between stud and staff members
ip6tables -A FORWARD -p tcp -s fd00:${a}:3:4::/64  -d fd00:${a}:3:1::/64 --dport 22 -j DROP
...
ip6tables -A FORWARD -p tcp -s fd00:${a}:3:1::/64 -d fd00:${a}:3:1::/64 --dport 22 -j ACCEPT

# Allow SNMP between stud and staff members
ip6tables -A FORWARD -p tcp -s fd00:${a}:3:4::/64  -d fd00:${a}:3:1::/64 -m multiport --dports 25,110,143 -j ACCEPT
...
ip6tables -A FORWARD -p tcp -s fd00:${a}:3:1::/64 -d fd00:${a}:3:1::/64 -m multiport --dports 25,110,143 -j ACCEPT

#Prohibit Router Advertisement for the student
ip6tables -A INPUT -p icmpv6 -s fd00:${a}:3:4::/64 --icmpv6-type 134/0 -j ACCEPT
...
ip6tables -A INPUT -p tcp -s fd00:${a}:3:1::/64  --dport 80 -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT
ip6tables -A INPUT -p tcp -s fd00:${a}:3:1::/64   --dport 8080 -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT
ip6tables -A INPUT -p tcp -s fd00:${a}:3:1::/64  --dport 443 -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT
...
ip6tables -A OUTPUT -p tcp -s fd00:${a}:3:1::/64  --dport 80 -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT
ip6tables -A OUTPUT -p tcp -s fd00:${a}:3:1::/64   --dport 8080 -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT
ip6tables -A OUTPUT -p tcp -s fd00:${a}:3:1::/64  --dport 443 -m conntrack --ctstate NEW,ESTABLISHED -j ACCEPT
...
ip6tables -L
vagrant@group3:~/test/lingi2142$ cat iptables/Pyth.sh 
#!/bin/bash
#Document generates by script firewall_config_creation.py, have a look on this python script for more details
...
ip6tables -A INPUT -p tcp -m multiport --dports 25,110,143 -j ACCEPT
#Print table from routers and display the rules added before
vagrant@group3:~/test/lingi2142$ cat iptables/Carn.sh
#!/bin/bash
#Document generates by script firewall_config_creation.py, have a look on this python script for more details
