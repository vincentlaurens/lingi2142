...
for a in 200 300
do
		ip6tables -A INPUT -i Halles-eth1 -p tcp --dport 22 -j ACCEPT
		ipt6ables -I OUTPUT -o  Halles-eth1 -p udp --dport 33434:33524 -m state --state NEW -j ACCEPT
		#Restrict incoming SSH to the local network
		ip6tables -A INPUT -i Halles-eth1 -p tcp -s fd00:${a}:3::1/48 --dport 22 -j ACCEPT
