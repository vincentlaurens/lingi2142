...
ip6tables -A INPUT -i Hall-eth1 -p tcp --destination-port 22 -j ACCEPT
ip6tables -A OUTPUT -o  Hall-eth1 -p udp --destination-port 33434:33524 -m state --state NEW -j ACCEPT
for a in 200 300
do
	#Restrict incoming SSH to the local network
	ip6tables -A INPUT -i Hall-eth1 -p tcp -s fd00:$a:3::1/48 --destination-port 22 -j ACCEPT
...
ip6tables -L > group3_cfg/Hall/log/iptables_log
echo [INFO SECURITY: Hall] Firewall set up!
