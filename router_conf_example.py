...
for router, configs in data.items():
    ...
    router_start_config = open(PATH+"group3_cfg/"+router+"_start", "w")
    ...
    file_stat = os.stat("group3_cfg/"+router+"_start")
    os.chmod("group3_cfg/"+router+"_start", file_stat.st_mode | stat.S_IEXEC)
    ...
    for vlan, location in configs["vlans"].items() :
        radvd_config = open("group3_cfg/"+router+"/radvd/"+router+".conf", "w")
        for vlan_use in VLAN_USES:
            radvd_config.write("""
